...
const REAUTH_HINT = "Session looks unauthenticated — run 'grafana-dash auth' to log in via Okta.";

/** One-time, headful Okta login. Holds the browser open until the user is done. */
export async function auth(config: Config): Promise<number> {
  if (config.headless) {
...
export async function pull(config: Config, arg?: string): Promise<number> {
  ...
  const session = await openSession(config);
  try {
    const res = await session.apiFetch<{ dashboard: DashboardModel }>(`/api/dashboards/uid/${uid}`);
