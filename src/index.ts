...
import { login, logout, pull, push, lint, shot, preview, setup, init, panelGet, panelSet, validate } from "./commands.ts";
...
const main = defineCommand({
  ...
  subCommands: {
    ...
    validate: defineCommand({
      meta: { name: "validate", description: "Run panel queries against the live Grafana datasource and report pass/fail" },
      args: {
        uid: { type: "positional" as const, description: "Dashboard UID or alias", required: false },
        panel: { type: "positional" as const, description: "Panel title or #<id> (omit to validate all panels)", required: false },
        var: { type: "string" as const, description: "Override template vars: --var cluster=prod,env=staging", required: false },
        verbose: { type: "boolean" as const, description: "Print the substituted query expression alongside each result", required: false },
        data: { type: "boolean" as const, description: "Print a summary of the returned data (series labels, last value, point count)", required: false },
        from: { type: "string" as const, description: "Query start time (default: now-1h). Grafana relative (now-6h) or epoch ms", required: false },
        to: { type: "string" as const, description: "Query end time (default: now)", required: false },
        raw: { type: "boolean" as const, description: "Dump the raw Grafana API response JSON and exit; useful for debugging --data output", required: false },
      },
      async run({ args }) {
        process.exit(await withConfig((config) => validate(config, args.uid, args.panel, args.var, args.verbose, args.data, args.from, args.to, args.raw)));
      },
    }),
    panel: defineCommand({
      ...
      subCommands: {
        get: defineCommand({
          meta: { name: "get", description: "Print panel JSON (or a specific field) to stdout; prints all matches if title is shared" },
          args: {
            uid: { type: "positional" as const, description: "Dashboard UID or alias", required: false },
            selector: { type: "positional" as const, description: "Panel title or #<id>", required: true },
            path: { type: "positional" as const, description: "Field path, e.g. gridPos.h or targets[0].expression", required: false },
          },
          async run({ args }) {
            process.exit(await withConfig((config) => panelGet(config, args.uid, args.selector, args.path)));
          },
        }),
        set: defineCommand({
          meta: { name: "set", description: "Set a panel field and write the local model back to disk; use #<id> if title is ambiguous" },
          args: {
            uid: { type: "positional" as const, description: "Dashboard UID or alias", required: false },
            selector: { type: "positional" as const, description: "Panel title or #<id>", required: true },
          ...
          async run({ args }) {
            process.exit(await withConfig((config) => panelSet(config, args.uid, args.selector, args.path, args.value)));
          },
        }),
      },
    }),
