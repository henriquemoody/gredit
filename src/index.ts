...
import { auth, pull, push, lint, shot } from "./commands.ts";
...
Commands:
  auth                 One-time headful Okta login; saves the session to the profile dir.
...
Typical loop:
  grafana-dash auth          # once, until the Okta session expires
...
async function main(): Promise<number> {
  ...
  const known = ["auth", "pull", "push", "lint", "shot"] as const;
